<template>
  <div :class="{ 'app-ready': appReady, 'app-loading': !appReady }">
    ...
    <template v-if="!isDashboard">
      <!-- WhatsApp -->
      <a :href="`https://wa.me/${cfg('whatsapp_number','966500000000')}`" target="_blank" class="fab fab--wa" title="واتساب">
      ...
      <transition name="fab-fade">
        <button v-if="isScrolled" class="fab fab--top" @click="scrollToTop" title="العودة للأعلى" aria-label="العودة للأعلى">
...
.fab:hover { transform: translateY(-3px); box-shadow: 0 8px 28px rgba(0,0,0,0.45); }
...
.fab--wa:hover { background: #20ba5a; }

.fab--top {
  bottom: 88px;
  background: linear-gradient(135deg, var(--neon-blue, #3b82f6), var(--neon-purple, #8b5cf6));
  color: white;
}

.fab-fade-enter-active, .fab-fade-leave-active { transition: all 0.3s; }
.fab-fade-enter-from, .fab-fade-leave-to { opacity: 0; transform: translateY(16px); }
